...
data['system'] = ['SmokedDuck' for _ in range(len(smokedduck_final))] + ['Logical-OPT' for _ in range(len(logical_final))]
...
p = ggplot(data, aes(x='Query', y='Runtime', condition='system', color='system', fill='system', group='system')) \
    ...
    + scale_y_continuous(
        ...
        breaks=[0, 60, 120, 180, 240, 300, 360],
        labels=[esc('0s'), esc('60s'), esc('120s'), esc('180s'), esc('240s'), esc('300s'), esc('360s')]
    ) \
    + scale_x_continuous(
